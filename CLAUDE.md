...
**More narrative detail, design notes, and roadmap context for clawdamp lives in `~/Documents/bjarne/projects/clawdamp/`.** Read files in that directory when you need background that isn't captured in code or in `docs/`. Treat it as the project's long-form knowledge base (goals, decisions, TODOs). If a question feels strategic rather than tactical, check there first.
...
- Site: https://clawdamp.stream
- Install: `curl -fsSL https://raw.githubusercontent.com/clawd-fi/clawdamp/HEAD/install.sh | sh`
...
| `pluginmgr/` | `clawdamp plugins install/remove/list` CLI: resolves GitHub/GitLab/Codeberg sources and a direct URL |
| `ipc/` | Unix-socket IPC: `server.go` listens; `client.go` + `protocol.go` drive subcommands like `clawdamp pause` from outside the TUI |
...
| `upgrade/` | Self-update logic for `clawdamp upgrade` |
| `site/` | Static website for clawdamp.stream — **keep synced with `docs/` on user-facing changes** |
...
```sh
make build        # go build -trimpath with version ldflags → ./clawdamp
make test         # go test ./...
make vet          # go vet ./...
...
Config lives at `~/.config/clawdamp/config.toml` (example at `config.toml.example`); plugins at `~/.config/clawdamp/plugins/`; custom radios at `~/.config/clawdamp/radios.toml`; themes at `~/.config/clawdamp/themes/`.
